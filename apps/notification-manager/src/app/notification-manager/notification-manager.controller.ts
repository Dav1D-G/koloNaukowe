...
import { RegistrationCommand } from '../cqrs/commands';
...
async accountRegistration(@Payload() contextRegistration: Dto) {
    ...
    const post = await this.commandBus.execute(new RegistrationCommand(contextRegistration.name,contextRegistration.email,contextRegistration.phone));
    return post;
}
